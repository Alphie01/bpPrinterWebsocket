...
# Zebra Yazıcı'nın USB Vendor ve Product ID'leri
VENDOR_ID = 0x0A5F
PRODUCT_ID = 0x0164

def find_zebra_printer():
    """Zebra yazıcıyı USB üzerinden bul ve bağlan."""
    device = usb.core.find(idVendor=VENDOR_ID, idProduct=PRODUCT_ID)
    
    if device is None:
        print("Zebra yazıcı bulunamadı.")
        return None

    print("Zebra yazıcı bulundu!")

    try:
        if device.is_kernel_driver_active(0):
            device.detach_kernel_driver(0)  # Kernel sürücüsünü serbest bırak
        
        device.set_configuration()
        print("Yazıcıya başarıyla bağlandı.")
        return device
    except usb.core.USBError as e:
        print(f"USB bağlantısı sırasında hata oluştu: {e}")
        return None

def send_zpl_to_printer(device, zpl_command):
    """Zebra yazıcıya ZPL komutunu gönder."""
    if device is None:
        print("Bağlı bir yazıcı bulunamadı!")
        return
    
    try:
        endpoint = device[0][(0, 0)][1]  # Çıkış endpoint'ini bul
        zpl_bytes = zpl_command.encode('utf-8')
        device.write(endpoint.bEndpointAddress, zpl_bytes, timeout=5000)
        print("ZPL komutu başarıyla gönderildi.")
    except usb.core.USBError as e:
        print(f"Yazıcıya veri gönderirken hata oluştu: {e}")

# Zebra yazıcıyı bul ve bağlan
printer = find_zebra_printer()


""" def send_zpl_to_printer(port: str, baudrate: int, zpl_command: str):
    try:
        with serial_output.Serial(port, baudrate, timeout=1) as ser:
...
# Örnek kullanım
serial_port = "USB001"  # Windows için (Linux/Mac için: "/dev/ttyUSB0" gibi)
baudrate = 9600
...
for obj in data:
    ...
    print(zpl_label)
    time.sleep(2)
    send_zpl_to_printer(printer, zpl_label)
